...
BOOL extractKeypairFromHex(NSString *hexKeypair, unsigned char *pk,
                           unsigned char *sk) {
  NSUInteger expectedLength =
      (crypto_box_SECRETKEYBYTES + crypto_box_PUBLICKEYBYTES) * 2;
  if (hexKeypair.length != expectedLength) {
    NSLog(@"❌ Hex keypair should be %lu hex chars (%lu bytes)",
          (unsigned long)expectedLength, (unsigned long)(expectedLength / 2));
    return NO;
  }

  unsigned char fullKey[crypto_box_SECRETKEYBYTES + crypto_box_PUBLICKEYBYTES];
  if (sodium_hex2bin(fullKey, sizeof(fullKey), [hexKeypair UTF8String],
                     hexKeypair.length, NULL, NULL, NULL) != 0) {
...
NSData *decryptSealedBox(NSData *sealedBox, NSString *hexKeypair) {
  ...
  int result = crypto_box_seal_open(
      m_bytes, c_bytes, (unsigned long long)sealedBox.length, pk, sk);
...
//     NSData *messageData = [messageString
//     dataUsingEncoding:NSUTF8StringEncoding];

//     NSData *encrypted = [sodium encryptMessage:messageData
//     recipientPublicKey:publicKey];

//     NSData *decrypted = [sodium decryptMessage:encrypted
//     recipientPublicKey:publicKey recipientSecretKey:secretKey];

//     NSString *decryptedString = [[NSString alloc] initWithData:decrypted
//     encoding:NSUTF8StringEncoding];
...
NSData *decodePayloadHex(NSString *payloadHex) {
  ...
  NSError *error = nil;
  NSDictionary *payloadBox = [NSJSONSerialization JSONObjectWithData:jsonData
                                                             options:0
                                                               error:&error];
  if (error || ![payloadBox isKindOfClass:[NSDictionary class]]) {
    NSLog(@"❌ Failed to parse JSON from payload_hex: %@",
...
- (void)pushRegistry:(PKPushRegistry *)registry
  ...
  // --- The system calls this method when a previously provided push token is
  // no longer valid for use. No action is necessary on your part to reregister
  // the push type. Instead, use this method to notify your server not to send
  // push notifications using the matching push token.
}
...
- (void)pushRegistry:(PKPushRegistry *)registry
  ...
  NSData *sealedBox = decodePayloadHex(
      payload.dictionaryPayload[@"payload"]); // your sealed message as hex
  ...
  if (sealedBox) {
    ...
    if (plaintext) {
      ...
      NSDictionary *jsonDict =
          [NSJSONSerialization JSONObjectWithData:plaintext
                                          options:0
                                            error:&jsonError];
  ...
  if (!handle) {
    handle = callerName ? callerName : @"Hugin Call";
  }

  // --- this is optional, only required if you want to call `completion()` on
  // the js side
  // [RNVoipPushNotificationManager addCompletionHandler:uuid
  ...
  // --- Delay completion to give JS 15 seconds of background execution to spin
  // up Swarm and WebRTC
