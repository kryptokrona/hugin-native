...
#include "keccak.h"
    ...
    void crypto_funcs::scReduce32(EllipticCurveScalar &scalar)
    {
        sc_reduce32(reinterpret_cast<unsigned char *>(&scalar));
    }

    void crypto_funcs::hashToScalar(const void *data, size_t length, EllipticCurveScalar &res)
    {
        hash_to_scalar(data, length, res);
    }

    void crypto_funcs::generate_keys(PublicKey &pub, SecretKey &sec)
    {
        ge_p3 point;
    ...
    void crypto_funcs::generate_deterministic_keys(PublicKey &pub, SecretKey &sec, SecretKey &second)
    {
        ge_p3 point;
        sec = second;
    ...
    SecretKey crypto_funcs::generate_m_keys(PublicKey &pub, SecretKey &sec, const SecretKey &recovery_key, bool recover)
    {
        ge_p3 point;
        SecretKey rng;
        if (recover)
    ...
    bool crypto_funcs::check_key(const PublicKey &key)
    {
        ge_p3 point;
        return ge_frombytes_vartime(&point, reinterpret_cast<const unsigned char *>(&key)) == 0;
    }

    bool crypto_funcs::secret_key_to_public_key(const SecretKey &sec, PublicKey &pub)
    {
        ge_p3 point;
    ...
    bool crypto_funcs::generate_key_derivation(const PublicKey &key1, const SecretKey &key2, KeyDerivation &derivation)
    {
        ge_p3 point;
        ge_p2 point2;
        ge_p1p1 point3;
    ...
    void
    crypto_funcs::derivation_to_scalar(const KeyDerivation &derivation, size_t output_index, EllipticCurveScalar &res)
    {
        struct
        {
    ...
    void crypto_funcs::derivation_to_scalar(
    ...
    bool crypto_funcs::derive_public_key(
        const KeyDerivation &derivation,
        size_t output_index,
    ...
    bool crypto_funcs::underive_public_key_and_get_scalar(
        const KeyDerivation &derivation,
        size_t output_index,
    ...
    void crypto_funcs::derive_secret_key(
        const KeyDerivation &derivation,
        size_t output_index,
    ...
    bool crypto_funcs::underive_public_key(
        const KeyDerivation &derivation,
        size_t output_index,
        const PublicKey &derived_key,
        PublicKey &base)
    ...
    void crypto_funcs::generate_signature(
    ...
    bool crypto_funcs::check_signature(const Hash &prefix_hash, const PublicKey &pub, const Signature &sig)
    {
        ge_p2 tmp2;
        ge_p3 tmp3;
    ...
    KeyImage crypto_funcs::scalarmultKey(const KeyImage &P, const KeyImage &a)
    {
        ge_p3 A;
        ge_p2 R;
    ...
    void crypto_funcs::hash_data_to_ec(const uint8_t *data, std::size_t len, PublicKey &key)
    {
        Hash h;
        ge_p2 point;
        ge_p1p1 point2;
    ...
    void crypto_funcs::generate_key_image(const PublicKey &pub, const SecretKey &sec, KeyImage &image)
    {
        ge_p3 point;
        ge_p2 point2;
        assert(sc_check(reinterpret_cast<const unsigned char *>(&sec)) == 0);
...
#pragma warning(disable : 4200)
    ...
    bool crypto_funcs::prepareRingSignatures(
    ...
    bool crypto_funcs::completeRingSignatures(
    ...
    bool crypto_funcs::generateRingSignatures(
        const Hash prefixHash,
        const KeyImage keyImage,
        const std::vector<PublicKey> publicKeys,
    ...
    bool crypto_funcs::checkRingSignature(
        const Hash &prefix_hash,
    ...
    void crypto_funcs::generateViewFromSpend(const Crypto::SecretKey &spend, Crypto::SecretKey &viewSecret)
    {
    ...
    void crypto_funcs::generateViewFromSpend(
    ...
    void crypto_funcs::generate_deterministic_subwallet_key(
