...
// const update_bare = rpc.register(6, {
//   request: ce.string,
//   response: ce.string,
// });

const file = rpc.register(6, {
  request: ce.string,
  response: ce.string,
});
...
export const bare = async (user, documentDirectoryPath) => {
  // await wallet();
  const data = JSON.stringify({ user, documentDirectoryPath });
  // Something is a bit fucky when passing two arguments, easy fix stringify for now
  init_bare.request(data);
};

export const update_bare_user = async (user) => {
  // update_bare.request(user);
};
