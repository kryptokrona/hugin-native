import type { ThemeBase, ThemeName, ThemeMode } from '@/types';
import { lightenHexColor } from '../services/utils/tools';

function generateMonochromaticColorTheme(accent: string): ThemeBase['color'] {
  return {
    accent,
...
const baseTheme = (mode: ThemeMode) => ({
  ...sharedColors[mode],
});
...
const createTheme = (
  name: ThemeName,
  overrides: {
    dark: Partial<ThemeBase['dark']>;
    light: Partial<ThemeBase['light']>;
    color: Partial<ThemeBase['color']>;
  }
): ThemeBase => ({
  dark: {
    ...baseTheme('dark'),
