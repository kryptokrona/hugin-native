...
const cmakeInstall = (buildOptions) => {
  const proc = spawnSync(
    cmake,
    ['--install', buildOptions.build, '--prefix', 'bare-libs'],
    {
      stdio: 'inherit',
    },
  );
...
async function action() {
  ...
  const hasDarwin =
    options.ios === true || Object.keys(options.iosSim).length > 0;
  const hasAndroid = Object.keys(options.android).length > 0;

  let ios = !hasDarwin && !hasAndroid ? true : options.ios === true;
  let iosSimArm64 =
    !hasDarwin && !hasAndroid ? true : options.iosSim.arm64 === true;
  let iosSimX64 =
    !hasDarwin && !hasAndroid ? true : options.iosSim.x64 === true;

  let androidArm64 =
    !hasAndroid && !hasDarwin ? true : options.android.arm64 === true;
  let androidArm =
    !hasAndroid && !hasDarwin ? true : options.android.arm === true;
  let androidX64 =
    !hasAndroid && !hasDarwin ? true : options.android.x64 === true;
  let androidIA32 =
    !hasAndroid && !hasDarwin ? true : options.android.ia32 === true;
